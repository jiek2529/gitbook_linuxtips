...
+ `git reset --mixed HEAD~1`  回退近一次提交，近一次修改在`未add`状态。
...
###HIGH-LEVEL COMMANDS (PORCELAIN) 高级命令 （精选）
       We separate the porcelain commands into the main commands and some
       ancillary user utilities.

####Main porcelain commands 精选命令
       git-add(1)
...
###LOW-LEVEL COMMANDS (PLUMBING) 低级命令 (探究)
...
####Internal helper commands 内部帮助命令
...
###FILE/DIRECTORY STRUCTURE 文件目录结构
